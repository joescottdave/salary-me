...
class Taxes extends React.Component {
  ...
  componentWillMount() {
    if (this.state.salary > 150000) {
      this.setState({
        additionalRate: true,
        compaverage: this.state.taxaverages.additional
      })
    } else if (this.state.salary > 45000) {
      this.setState({
        higherRate: true,
        compaverage: this.state.taxaverages.higher
      })
    } else {
      this.setState({
        basicRate: true,
        compaverage: this.state.taxaverages.basic
      })
    }
  }
  render() {
      ...
      <Card>
        <h2>
          <span className="highlight">
            In 2014-15 <br /> the&nbsp;
            {this.state.additionalRate
              ? this.state.peoplePercents.additional
              : null}
            ...
            {this.state.additionalRate
              ? this.state.incomePercents.additional
              : null}
        ...
        <h3>
          That's roughly £
          {this.state.additionalRate
            ? this.state.incomeTotals.additional
            : null}
        ...
        <h2>
          <span className="highlight">
            And paid&nbsp;
            {this.state.additionalRate
              ? this.state.taxpercents.additional
              : null}
        ...
        <h3>
          ...
          {this.state.tax < this.state.compaverage ? (
            <span className="highlight">lower</span>
          ) : (
            <span className="highlight">higher</span>
          )}{' '}
          than the average payment of £
          {this.state.compaverage
            .toString()
            .replace(/\B(?=(\d{3})+(?!\d))/g, ',')}{' '}
          for this bracket.
        </h3>
