...
class IncomesVis extends React.Component {
  constructor(props) {
    super(props)
    this.setState = { salary: props.salary, width: props.width }
  }
  ...
  drawIncomeGraph() {
    ...
    d3.queue()
      ...
      .await(function(error, data) {
        ...
        var hoz_lines = context.append('g').attr('class', 'hoz_lines')

        var hoz_line = hoz_lines
          .selectAll('.hoz_line')
        ...
        var linemarks = context.append('g').attr('class', 'linemarks')

        var linemark = linemarks.selectAll('.linemark').data(data)
        ...
        linemark
          ...
          .attr('height', function(d) {
            return height - y(d['2014-15'])
          })
          .attr('y', function(d) {
            return y(d['2014-15'])
          })
          .attr('fill', function(d) {
            return color(d['Percentile'])
          })
          .attr('opacity', 1)
        ...
        context
          ...
          .attr('text-anchor', function() {
            if (
              (percent(salary) >= 33 && percent(salary) <= 50) ||
              percent(salary) > 90
            ) {
              marg = -4
              return 'end'
