...
class PieVis extends Component {
    ...
    setContext() {
        ...
        return d3.select(this.refs.PieVis).append('svg')
            .attr('class', 'pie-vis')
            .attr('height', height)
            .attr('width', width);
    }
    ...
    var data = [{"name": "Income Tax", "value": main.props.tax/12},{"name": "National Insurance", "value": main.props.contribution/12}, {"name": "Take Home", "value": main.props.takehome}];
    ...
    var pieText = arc.append('text')
        .attr('class', 'pie-text')
        .text("Paycheque");

    var pietextrect = document.querySelector('.pie-text').getBoundingClientRect();
    ...
    function hoverPie(d) {
        ...
        pieText.remove();
        pieText = g.append('text').attr('class','pie-text').attr('fill', textcolor).text(text);
        var pietextrect = document.querySelector('.pie-text').getBoundingClientRect();
        pieText.attr('transform', 'translate(' + -pietextrect.width/2 + ',0)');
    };
    ...
    function offHoverPie(d) {
        ...
        pieText.remove();
        pieText = g.append('text').attr('class','pie-text').attr('fill', 'black').text('Paycheque');
        var pietextrect = document.querySelector('.pie-text').getBoundingClientRect();
        pieText.attr('transform', 'translate(' + -pietextrect.width/2 + ',0)');
    };
